table 5110309 "POI Item Attribute 7"
{

    Caption = 'Conservation';
    // DrillDownFormID = Form5110424;
    // LookupFormID = Form5110424;
    ...
    begin
        lrc_Item.SETRANGE("POI Item Attribute 7", Code);
        IF NOT lrc_Item.ISEMPTY() THEN
            ERROR(ADF_GT_TEXT002Txt);

        lrc_ItemLedgerEntry.SETRANGE("POI Item Attribute 7", Code);
        IF NOT lrc_ItemLedgerEntry.ISEMPTY() THEN
            ERROR(ADF_GT_TEXT003Txt);

        lrc_BatchVariant.SETRANGE("Item Attribute 7", Code);
